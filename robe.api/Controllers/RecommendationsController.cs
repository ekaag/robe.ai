...
public class RecommendationsController : ControllerBase
{
    ...
    public async Task<IActionResult> Recommend([FromBody] RecommendRequest request, CancellationToken ct)
    {
        ...
        if (request.MaxBudget is < 0)
            return BadRequest(new { error = "maxBudget must be non-negative." });

        var userId  = _currentUser.UserId;
        var profile = await _profileRepo.GetAsync(userId, ct);
        if (profile is null)
            return NotFound(new { error = "No style profile found. Run POST /api/users/me/profile/generate first." });
        ...
        {
            recommendations,
            profileGeneratedAt = profile.ModifiedAt,
