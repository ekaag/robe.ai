...
public class HttpContextCurrentUser : ICurrentUser
{
    ...
    public string UserId =>
        _accessor.HttpContext?.User.FindFirstValue("oid")                    // Entra JWT (MapInboundClaims=false)
        ?? _accessor.HttpContext?.User.FindFirstValue("sub")                 // OIDC subject fallback
        ?? _accessor.HttpContext?.User.FindFirstValue(ClaimTypes.NameIdentifier) // LocalAuthHandler
